...
import { getLessonById, type SavedLesson } from "@/utils/lessonStorage";

const featuredPlanDocxUrl =
  "https://cdn.builder.io/o/assets%2Fa061ccedc21643e89c15d64ceb68a9d5%2F53615fcbc8d94005bf91d1a8d8587a4b?alt=media&token=4dd390ad-2424-4786-b90e-35f8367e65e6&apiKey=a061ccedc21643e89c15d64ceb68a9d5";
const featuredSongDownloadUrl =
  "https://cdn.builder.io/o/assets%2Fa061ccedc21643e89c15d64ceb68a9d5%2Feab0b9de30b642ffbf9f9a769536b92f?alt=media&token=8e1625ac-8b26-4155-9633-54684726e0aa&apiKey=a061ccedc21643e89c15d64ceb68a9d5";
...
[Verse 2]
ตื่นเช้าหกโมงครึ่ง อาบน้ำหกสี่ห้า
เจ็ดโมงกินข้าว เจ็ดโมงครึ่งออกเดิน
เรียงลำดับเวลา ทำตามได้ไม่ยาก
ชีวิตเป็นระเบียบ สบายใจทุกวัน
...
const electronicsContent = `โครงสร้างอะตอม

โครงสร้างอะตอมเป็นพื้นฐานในการเข้าใจสารกึ่งตัวนำและการทำงานของอุปกรณ์อิเล็กทรอนิกส์

ส่วนประกอบหลักของอะตอม

1. นิวเคลียส (Nucleus)
   - ประกอบด้วยโปรตอน (Proton) มีประจุบวก
   - และนิวตรอน (Neutron) ไม่มีประจุ
   - มีขนาดเล็กมากแต่มีมวลมากกว่า 99% ของอะตอม

2. อิเล็กตรอน (Electron)
   - มีประจุลบ
   - โคจรรอบนิวเคลียสในระดับพลังงานต่างๆ (Shells)
   - มีมวลเบามากเมื่อเทียบกับโปรตอน

หลักการทำงาน

- อิเล็กตรอนในวงนอกสุด (Valence Electrons) มีบทบาทสำคัญในการสร้างพันธะกับอะตอมอื่น
- ในสารกึ่งตัวนำ อิเล็กตรอนเหล่านี้สามารถหลุดออกจากอะตอมได้ง่ายเมื่อได้รับพลังงาน

ระดับพลังงาน (Energy Levels)

- อิเล็กตรอนสามารถมีอยู่ได้เฉพาะในระดับพลังงานที่กำหนด
- ระดับแรก (K shell) เก็บได้ 2 อิเล็กตรอน
- ระดับที่สอง (L shell) เก็บได้ 8 อิเล็กตรอน
- และต่อไปเรื่อยๆ
- อิเล็กตรอนจะเลื่อนไประดับสูงขึ้นเมื่อได้รับพลังงาน และตกลงมาเมื่อปล่อยพลังงาน

สารกึ่งตัวนำ (Semiconductors)

- เช่น ซิลิกอน (Silicon) และเจอร์มาเนียม (Germanium)
- มีอิเล็กตรอนวาเลนซ์ 4 ตัว
- สามารถนำไฟฟ้าได้บ้างในสภาวะปกติ

ความสำคัญในอิเล็กทรอนิกส์

การเข้าใจโครงสร้างอะตอมช่วยให้เราเข้าใจ:
- การไหลของกระแสไฟฟ้า
- การทำงานของไดโอด ทรานซิสเตอร์ และชิปทั่วไป
...
export default function SavedLessonViewPage() {
  ...
  const isTimeLessonFeatured = useMemo(
    () =>
      lesson &&
      lesson.subject === timeLessonSelection.subject &&
      lesson.unit === timeLessonSelection.unit &&
      lesson.topic === timeLessonSelection.topic,
    [lesson],
  );
  ...
  const isScienceLessonFeatured = useMemo(
    () =>
      lesson &&
      lesson.subject === scienceLessonSelection.subject &&
      lesson.unit === scienceLessonSelection.unit &&
      lesson.topic === scienceLessonSelection.topic,
    [lesson],
  );
  ...
  const isEnglishLessonFeatured = useMemo(
    () =>
      lesson &&
      lesson.subject === englishLessonSelection.subject &&
      lesson.unit === englishLessonSelection.unit &&
      lesson.topic === englishLessonSelection.topic,
    [lesson],
  );
  ...
  const isElectricalLessonFeatured = useMemo(
    () =>
      lesson &&
      lesson.major === electricalLessonSelection.major &&
      lesson.subject === electricalLessonSelection.subject &&
      lesson.unit === electricalLessonSelection.unit &&
      lesson.topic === electricalLessonSelection.topic,
    [lesson],
  );
  ...
  const isElectricalLessonFormat2 = useMemo(
    () =>
      ...
      (lesson?.content.lessonContent?.includes("แบบที่ 2 — เล่าเรื่อง") ?? false),
    [lesson, isElectricalLessonFeatured],
  );

  const isElectronicsLessonFeatured = useMemo(
    () =>
      lesson &&
      lesson.major === electronicsLessonSelection.major &&
      lesson.subject === electronicsLessonSelection.subject &&
      lesson.unit === electronicsLessonSelection.unit &&
      lesson.topic === electronicsLessonSelection.topic,
    [lesson],
  );
  ...
  const isElectronicsLessonFormat2 = useMemo(
    () =>
      ...
      (lesson?.content.lessonContent?.includes("แบบที่ 2 — เล่าเรื่อง") ?? false),
    [lesson, isElectronicsLessonFeatured],
  );
      ...
      <DashboardLayout>
        <div className="mx-auto max-w-4xl space-y-6">
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border bg-muted/10 py-16 text-center">
            <p className="text-lg font-medium text-foreground">ไม่พบแผนการสอน</p>
            <Button
              onClick={() => navigate("/dashboard/lesson-plan")}
      ...
      case "lesson-content":
            ...
            <div className="space-y-4 rounded-2xl border border-border bg-background p-6 shadow-sm">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <h3 className="mt-1 text-xl font-bold text-foreground">{lesson.planName}</h3>
                  <p className="mt-2 text-sm text-muted-foreground">เนื้อหาที่ AI สร้างขึ้น</p>
                </div>
              </div>
        ...
        return (
          <div className="space-y-4 rounded-2xl border border-border bg-background p-6 shadow-sm">
            ...
            <div className="max-h-[600px] overflow-y-auto rounded-2xl border bg-muted/10 p-6">
              <div className="whitespace-pre-wrap text-sm leading-8 text-foreground font-normal">
                {shouldShowFeaturedMockup ? contentToDisplay : lesson.content.lessonContent || "เนื้อหาการเรียนไม่พบ"}
              </div>
            </div>
            ...
            <div className="space-y-4 rounded-2xl border border-border bg-background p-4 shadow-sm">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <h3 className="mt-1 text-xl font-bold text-foreground">{lesson.planName}</h3>
                  <p className="mt-2 text-sm text-muted-foreground">วิดีโอการสอนแบบมีความคิดสร้างสรรค์</p>
                </div>
              </div>

              <div className="flex items-center justify-between gap-3 rounded-2xl border bg-muted/20 px-4 py-3">
                <div className="space-y-0.5">
                  <div className="text-sm font-semibold text-foreground">วีดีโอเรื่อง {lesson.planName}</div>
                  <div className="text-xs text-muted-foreground">วิดีโอการสอนแบบมีความคิดสร้างสรรค์</div>
                </div>
                <a
                  href={videoUrl}
                  target="_blank"
        ...
        return (
          <div className="space-y-4 rounded-2xl border border-border bg-background p-4 shadow-sm">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-sm font-medium text-muted-foreground">วีดีโอ</p>
                <h3 className="mt-1 text-xl font-bold text-foreground">วีดีโอเรื่อง {lesson.planName}</h3>
                <p className="mt-2 text-sm text-muted-foreground">วิดีโอการสอนแบบมีความคิดสร้างสรรค์</p>
              </div>
            </div>
            ...
            {shouldShowFeaturedMockup ? (
              ...
              <div className="flex items-center justify-center rounded-2xl bg-muted/30 p-8">
                <p className="text-sm text-muted-foreground">ตัวอย่างวิดีโอการสอน</p>
              </div>
            )}
          </div>
        );
        ...
        if ((isElectricalLessonFeatured || isElectronicsLessonFeatured) && tabValue === "slides-lesson") {
          return (
            <div className="space-y-4 rounded-2xl border border-border bg-background p-4 shadow-sm">
              ...
              <div className="flex items-center justify-between gap-3 rounded-2xl border bg-muted/20 px-4 py-3">
                <div className="space-y-0.5">
                  <div className="text-sm font-semibold text-foreground">สไลด์การสอนเรื่อง {lesson.planName}</div>
        ...
        return (
          <div className="space-y-4 rounded-2xl border border-border bg-background p-4 shadow-sm">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-sm font-medium text-muted-foreground">สไลด์การสอน</p>
                <h3 className="mt-1 text-xl font-bold text-foreground">สไลด์การสอนเรื่อง {lesson.planName}</h3>
                <p className="mt-2 text-sm text-muted-foreground">หน้าปกและเป้าหมายการเรียนรู้</p>
              </div>
            </div>
        ...
        return (
          <div className="space-y-4 rounded-2xl border border-border bg-background p-4 shadow-sm">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-sm font-medium text-muted-foreground">สไลด์กิจกรรม</p>
                <h3 className="mt-1 text-xl font-bold text-foreground">สไลด์กิจกรรมเรื่อง {lesson.planName}</h3>
                <p className="mt-2 text-sm text-muted-foreground">รูปแบบกิจกรรมการเรียน</p>
              </div>
            </div>
          ...
          return (
            <div className="space-y-4 rounded-2xl border border-border bg-background p-4 shadow-sm">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between gap-3 rounded-2xl border bg-muted/20 px-4 py-3">
                  <div className="space-y-0.5">
                    <div className="text-sm font-semibold text-foreground">แผนการจัดการเรียนรู้เรื่อง {lesson.planName} - กิจกรรมแบบที่ 1</div>
                    <div className="text-xs text-muted-foreground">{lesson.major} · {lesson.subject}</div>
                  </div>
                  <a
                    href={plan1Url}
                    target="_blank"
                ...
                <div className="flex items-center justify-between gap-3 rounded-2xl border bg-muted/20 px-4 py-3">
                  <div className="space-y-0.5">
                    <div className="text-sm font-semibold text-foreground">แผนการจัดการเรียนรู้เรื่อง {lesson.planName} - กิจกรรมแบบที่ 2</div>
                    <div className="text-xs text-muted-foreground">{lesson.major} · {lesson.subject}</div>
                  </div>
                  <a
                    href={plan2Url}
        ...
        return (
          <div className="space-y-4 rounded-2xl border border-border bg-background p-4 shadow-sm">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-sm font-medium text-muted-foreground">แผนการจัดการเรียนรู้</p>
                <h3 className="mt-1 text-xl font-bold text-foreground">แผนการสอนเรื่อง {lesson.planName}</h3>
                <p className="mt-2 text-sm text-muted-foreground">แผนการจัดการเรียนรู้ฉบับสมบูรณ์</p>
              </div>
            </div>
            ...
            {shouldShowFeaturedMockup ? (
              <div className="flex items-center justify-between gap-3 rounded-2xl border bg-muted/20 px-4 py-3">
                <div className="space-y-0.5">
                  <div className="text-sm font-semibold text-foreground">แผนการสอนเรื่อง {lesson.planName}</div>
                  <div className="text-xs text-muted-foreground">{lesson.major} · {lesson.subject}</div>
                </div>
                <div className="flex gap-2">
                  <a
        ...
        return (
          <div className="space-y-4 rounded-2xl border border-border bg-background p-4 shadow-sm">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-sm font-medium text-muted-foreground">แบบฝึกหัด</p>
                <h3 className="mt-1 text-xl font-bold text-foreground">แบบฝึกหัดเรื่อง {lesson.planName}</h3>
                <p className="mt-2 text-sm text-muted-foreground">ดาวน์โหลดแบบฝึกหัดและเฉลย</p>
              </div>
            </div>
          ...
          <div className="space-y-4 rounded-2xl border border-border bg-background p-4 shadow-sm">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h3 className="mt-1 text-xl font-bold text-foreground">เพลงเรื่อง {lesson.planName}</h3>
              ...
              <>
                <div className="rounded-2xl border bg-muted/20 p-4">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="text-sm font-semibold text-foreground">{cleanTopicName(lesson.topic)}</p>
          ...
          <div className="space-y-4 rounded-2xl border border-border bg-background p-4 shadow-sm">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h3 className="mt-1 text-xl font-bold text-foreground">เกมเรื่อง {lesson.planName}</h3>
            ...
            {shouldShowFeaturedMockup ? (
              <>
                <div className="rounded-2xl border bg-muted/20 p-4">
                  <p className="text-sm font-semibold text-foreground">เกมเรื่อง {lesson.planName}</p>
    ...
    <DashboardLayout>
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <div className="space-y-2">
          <h1 className="text-3xl font-bold text-foreground">{lesson.planName}</h1>
